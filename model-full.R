run.model <- function(d,
  ...
  model.jags <- function() {
    ...
    ## random effect of enclosure
    sigma.phi.enc ~ dunif(0,20)
    tau.phi.enc <- 1/(sigma.phi.enc*sigma.phi.enc)
    for(enclosure in 1:nenc) {
      phi.enc[enclosure] ~ dnorm(0, tau.phi.enc)
    }
